<x-app-layout>
    <x-slot name="title">Upload {{ $meta['title'] }}</x-slot>
    <x-slot name="crumbs">
        Upload Data / {{ $meta['title'] }}
    </x-slot>
    ...
    <div class="card mb-4" style="border-top:4px solid var(--danger);">
        <div class="card-body" style="display:flex; flex-wrap:wrap; gap:16px; align-items:center; justify-content:space-between;">
            <div>
                ...
                <h2 style="margin:6px 0 8px; font-size:26px; color:var(--ink);">Upload {{ $meta['title'] }}</h2>
                <div class="text-muted" style="max-width:760px; font-size:14px; line-height:1.7;">{{ $meta['description'] }} The file is stored in its own folder and saved as its own database record.</div>
            </div>
            <div class="kpi" style="min-width:170px;">
                <div class="label">Upload type</div>
                <div class="value">{{ $meta['title'] }}</div>
                <div class="text-muted" style="font-size:13px; margin-top:4px;">Single file</div>
            </div>
        </div>
    </div>
    ...
    <div class="grid cols-3 mb-6">
        <div style="grid-column: span 2;">
            <form method="POST" action="{{ route('upload.type.store', $type) }}" enctype="multipart/form-data" id="uploadForm">
                @csrf
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        @if ($errors->has($fieldName))
                            <div style="border:1px solid #FCA5A5; background:#FEF2F2; color:#991B1B; border-radius:8px; padding:12px 14px; margin-bottom:16px; font-size:13px;">{{ $errors->first($fieldName) }}</div>
                        @endif
                        <div class="input-row">
                            <div class="field">
                        ...
                        <div class="text-muted" style="font-size:13px; margin-top:10px;">Pick the fiscal year and Nepali month, then attach the {{ $meta['title'] }} file for that period.</div>
                    </div>
                </div>

                <div class="card mt-4">
                    <div class="card-head">
                        <h2>Upload {{ $meta['title'] }} file</h2>
                        <!-- <span class="badge warning">UI + backend</span> -->
                    </div>
                    <div class="card-body">
                        <div class="card" data-upload-card="{{ $type }}" style="border:1px solid var(--line); box-shadow:none; background:#fff; border-top:3px solid var(--danger);">
                            <div class="card-body" style="display:grid; gap:14px;">
                                <div class="flex between center" style="gap:12px; align-items:flex-start;">
                                    <div>
                                        <div style="font-size:18px; font-weight:700; color:var(--ink);">{{ $meta['title'] }}</div>
                                        <div class="text-muted" style="font-size:13px; margin-top:4px; line-height:1.6;">{{ $meta['description'] }}</div>
                                    </div>
                                    <span class="badge info" data-upload-badge>Pending</span>
                                </div>

                                <label class="dropzone" data-dropzone data-target="{{ $type }}" style="min-height:160px; border-style:dashed; background:#fafafa;">
                                    <input type="file" name="{{ $fieldName }}" id="{{ $fieldName }}" data-upload-input data-target="{{ $type }}" accept="{{ $meta['accept'] }}" hidden>
                                    ...
                                    <h3 id="{{ $type }}_title" style="margin:0;">Drop {{ $meta['title'] }} file here</h3>
                                    <p id="{{ $type }}_text" style="margin:0;">or click to browse - Accepted: {{ str_replace(',', ', ', $meta['extensions']) }}</p>
                                </label>

                                <div class="flex between center" style="gap:12px;">
                                    <div class="text-muted" id="{{ $type }}_meta" style="font-size:13px; overflow-wrap:anywhere;">No file selected</div>
                                    <button type="button" class="btn btn-ghost btn-sm" data-clear-upload="{{ $type }}" style="border:1px solid var(--line);">Clear</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card mt-4">
                    ...
                    <div class="card-body">
                        ...
                        @else
                            <div class="text-muted" style="font-size:13px;">Upload a file first. Once it is stored, the database import module will be ready.</div>
                            <div class="flex gap-3 mt-4">
        ...
        <div>
                ...
                <div class="card-body" style="padding:0;">
                    <table class="t">
                        ...
                        <tbody>
                            <tr>
                                <td>{{ $meta['title'] }}</td>
                                <td class="text-muted" data-selected-file="{{ $type }}">No file selected</td>
                                <td><span class="badge info" data-selected-status="{{ $type }}">Pending</span></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
            ...
            <div class="card">
                <div class="card-head">
                    <h2>Saved {{ $meta['title'] }} uploads</h2>
                </div>
                <div class="table-wrap" style="border:none; border-radius:0;">
                    <table class="t">
                        ...
                        <tbody>
                            ...
                            @empty
                                <tr>
                                    <td colspan="4" class="text-muted">No {{ $meta['title'] }} uploads yet.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
            ...
            <div class="card mt-4">
                ...
                <div class="card-body" style="font-size:13.5px; color: var(--ink-soft); line-height:1.7;">
                    <ul style="list-style:disc outside; padding-left:22px; margin:0; display:grid; gap:6px; color:var(--brand);">
                        <li><span style="color:var(--ink-soft);">This page only accepts {{ $meta['title'] }} files.</span></li>
                        ...
                        <li><span style="color:var(--ink-soft);">Accepted formats: {{ str_replace(',', ', ', $meta['extensions']) }}.</span></li>
                    </ul>
                </div>
            </div>
        </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            const key = {{ \Illuminate\Support\Js::from($type) }};
            const formatSize = bytes => `${(bytes / 1024 / 1024).toFixed(2)} MB`;

            const updateCard = (file) => {
                ...
                if (!file) {
                    ...
                    text.textContent = 'or click to browse - Accepted: {{ str_replace(',', ', ', $meta['extensions']) }}';
